...
export const DashboardChartsSection: React.FC<DashboardChartsSectionProps> = ({
  ...
  let maxVal = 10000;
  if (maxDataAmount > 0) {
    if (maxDataAmount <= 5000) maxVal = 5000;
  ...
  const currentFilterLabel =
    timeFilterOptions.find((opt) => opt.key === selectedTimeFilter)?.label || 'This Week';
      ...
      <Pressable style={styles.card} onPress={onPressRevenue}>
        ...
        <View style={styles.revenueRow}>
          ...
          <View
            ...
            <Ionicons
              name={growthPct >= 0 ? 'arrow-up' : 'arrow-down'}
              size={11}
              color={growthPct >= 0 ? '#059669' : Colors.rose}
            />
            <Text
              style={[
                styles.trendText,
