...
export default function LoginScreen() {
    ...
    const handleSendOtp = async () => {
        if (!agreedToTerms) {
            Alert.alert('Agreement Required', 'Please check the box to agree to our Terms of Service and Privacy Policy before continuing.');
            return;
        }
    ...
    const handleVerifyOtp = async () => {
        if (!agreedToTerms) {
            Alert.alert('Agreement Required', 'Please agree to our Terms of Service and Privacy Policy to continue.');
            return;
        }
        if (!otp || otp.length < 6) {
    ...
    return (<KeyboardAvoidingView style={[styles.container, { paddingTop: Math.max(insets.top, 24), paddingBottom: Math.max(insets.bottom, 16) }]} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.cardInner}>
            ...
            {!isOtpSent ? (<>
            ...
            ]} disabled={isLoading} onPress={handleSendOtp}>
                ...
                styles.primaryBtn,
                { opacity: pressed || isLoading ? 0.88 : 1 },
            ]} disabled={isLoading} onPress={handleVerifyOtp}>
                  <Text style={styles.primaryBtnText}>
                    {isLoading ? 'Verifying...' : 'Verify & Continue'}
                  </Text>
                </Pressable>

                <Pressable style={styles.changePhoneBtn} onPress={() => {
                setIsOtpSent(false);
                setOtp('');
            }}>
        ...
        <View style={styles.legalSection}>
          <Pressable style={styles.checkboxRow} onPress={() => setAgreedToTerms((prev) => !prev)}>
            <View style={[
            styles.checkboxBox,
