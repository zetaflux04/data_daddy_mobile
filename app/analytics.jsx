...
export default function AnalyticsScreen() {
    ...
    return (<View style={styles.container}>
      <AppHeader title="Profit & Loss" rightAction={<Pressable style={({ pressed }) => [styles.headerAddBtn, { opacity: pressed ? 0.7 : 1 }]} onPress={() => setIsAddExpenseOpen(true)}>
            <Ionicons name="add" size={18} color={Colors.primary}/>
            <Text style={styles.headerAddText}>Add</Text>
          </Pressable>}/>

      <ScrollView style={styles.scrollArea} contentContainerStyle={[styles.content, { paddingBottom: Math.max(insets.bottom, 24) + 16 }]} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.expenseSectionHeader}>
          ...
          <Pressable style={({ pressed }) => [styles.addExpBtn, { opacity: pressed ? 0.88 : 1 }]} onPress={() => setIsAddExpenseOpen(true)}>
            <Ionicons name="add" size={16} color="#FFFFFF"/>
            <Text style={styles.addExpBtnText}>Add Expense</Text>
          </Pressable>
        </View>

        {expenses.length === 0 ? (<View style={styles.emptyCard}>
          ...
          </View>) : (expenses.map((exp) => (<View key={exp._id} style={styles.expenseItemCard}>
      ...
      {/* Add Expense Modal */}
      <Modal visible={isAddExpenseOpen} transparent animationType="slide" statusBarTranslucent onRequestClose={() => setIsAddExpenseOpen(false)}>
          ...
          <View style={[styles.modalCard, { paddingBottom: Math.max(insets.bottom, 20) + 12 }]}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} bounces={false}>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.catScroll}>
                {categories.map((c) => (<Pressable key={c.key} style={[styles.catChip, expCategory === c.key && styles.catChipActive]} onPress={() => setExpCategory(c.key)}>
