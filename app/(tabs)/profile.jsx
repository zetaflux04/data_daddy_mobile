...
const MenuItem = ({ icon, title, subtitle, badge, badgeColor = Colors.primary, iconBg = '#F1F5F9', iconColor = '#475569', onPress, destructive = false, }) => (<Pressable onPress={onPress} style={({ pressed }) => [
        styles.menuItem,
    ...
    {badge ? (<View style={[styles.badgePill, { backgroundColor: `${badgeColor}18` }]}>
        <Text style={[styles.badgeText, { color: badgeColor }]}>{badge}</Text>
      </View>) : null}

    <Ionicons name="chevron-forward" size={18} color={destructive ? Colors.rose : '#CBD5E1'} style={{ marginLeft: 6 }}/>
  </Pressable>);
    ...
    return (<ScrollView style={styles.container} contentContainerStyle={[styles.content, { paddingBottom: Math.max(insets.bottom, 24) + 24 }]} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.shopCard}>
        <View style={styles.shopAvatarWrapper}>
          ...
          {isUploadingPhoto ? (<ActivityIndicator size="small" color="#FFFFFF"/>) : shop?.logoUrl && !avatarFailed ? (() => {
          ...
          <Pressable style={({ pressed }) => [styles.avatarCameraBtn, { opacity: pressed ? 0.8 : 1 }]} onPress={handlePickImage} disabled={isUploadingPhoto}>
            {isUploadingPhoto ? (<ActivityIndicator size="small" color="#FFFFFF"/>) : (<Ionicons name="camera" size={14} color="#FFFFFF"/>)}
          </Pressable>
        </View>
      ...
      <View style={styles.section}>
        <View style={styles.menuGroup}>
          <MenuItem icon="log-out-outline" iconBg="rgba(239, 68, 68, 0.12)" iconColor={Colors.rose} title="Sign Out" subtitle="Log out from this device" destructive onPress={handleLogout}/>
        </View>
      </View>
