    ...
    return (<View style={styles.container}>
      ...
      <View style={styles.searchContainer}>
        <View style={styles.searchBox}>
          ...
          <TextInput style={styles.searchInput} placeholder="Search Job ID, customer, phone, model..." placeholderTextColor="#94A3B8" value={searchQuery} onChangeText={setSearchQuery}/>
          {searchQuery.length > 0 && (<Pressable onPress={() => setSearchQuery('')}>
              <Ionicons name="close-circle" size={18} color="#94A3B8"/>
            </Pressable>)}
        </View>

        <Pressable style={({ pressed }) => [styles.createBtn, { opacity: pressed ? 0.88 : 1 }]} onPress={() => router.push('/job/new')}>
      ...
      <View style={styles.filterScrollWrapper}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filterContainer}>
          {statusTabs.map((tab) => {
            ...
            return (<Pressable key={tab.key} onPress={() => setSelectedStatus(tab.key)} style={[
                    styles.filterChip,
      ...
      {/* Job Cards List */}
      <FlatList data={jobs} keyExtractor={(item) => item._id} contentContainerStyle={styles.listContent} refreshing={isLoading} onRefresh={fetchJobs} renderItem={({ item }) => (<JobCardItem job={item} onPress={() => router.push(`/job/${item._id}`)}/>)} ListEmptyComponent={<View style={styles.emptyState}>
